...
interface Row { symbol: string; name: string; tier: "core" | "large" | "growth"; priceUsd: number | null; referencePriceUsd: number | null; premiumPct: number | null; change24hPct: number | null; change30dPct: number | null; volatility30dPct: number | null; liquidityUsd: number | null; volume24hUsd: number | null; hopCostPct: number | null; protocol: string | null; logoUrl: string | null }

const TIER_LABEL = { core: "Index & mega cap", large: "Large cap", growth: "Small & mid cap" };

export default function MarketPage() {
  const { data, isLoading, error } = useQuery({ queryKey: ["universe"], queryFn: async () => (await fetch("/api/universe")).json() as Promise<{ scannedAt: string; total: number; assets: Row[] }>, staleTime: 60_000 });
  const [tier, setTier] = useState<"all" | "core" | "large" | "growth">("all");
  const [sort, setSort] = useState<"liquidity" | "change24h" | "change30d" | "premium">("liquidity");
  const rows = (data?.assets ?? []).filter((r) => tier === "all" || r.tier === tier).sort((a, b) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-[60ch] text-ink-2">Every tokenized stock with a real on-chain market, priced live against its Chainlink reference. Conservative sees the first group, Balanced adds the second, Aggressive sees everything.</p>
      </div>
      <div className="flex flex-wrap items-center gap-3">
        <div className="seg" role="group" aria-label="Group">
          {(["all", "core", "large", "growth"] as const).map((t) => <button key={t} type="button" aria-pressed={tier === t} onClick={() => setTier(t)}>{t === "all" ? "All" : TIER_LABEL[t]}</button>)}
        </div>
            ...
            <tbody className="tnum">
              {rows.map((r) => (
                <tr key={r.symbol} className="border-t border-line">
                  <td className="max-w-[260px] px-4 py-2.5"><span className="font-semibold">{r.symbol}</span> <span className="ml-1 truncate text-sm text-ink-3">{r.name.length > 22 ? r.name.slice(0, 22) + "…" : r.name}</span><span className="ml-2 whitespace-nowrap text-xs text-ink-3">{TIER_LABEL[r.tier]}</span></td>
                  ...
                  <td className="pr-4 text-right">{r.hopCostPct == null ? "—" : pct(r.hopCostPct, 2, false)}</td>
                </tr>
              ))}
            </tbody>
          </table>
